    ...
    with torch.no_grad():
        for feature, target, slices, targets_org in testloader:       
            ...
            if c.dataset['split_wav_using_overlapping']:
                ...
                if slices is not None and targets_org is not None:
                    ...
                    for i in range(slices.size(0)):
                        num_samples = int(slices[i].cpu().numpy()) #aqui mora o problema slices nao funfa com float
                    ...
                    target = target.reshape(-1).float().cpu().numpy().tolist()
                    target = list(np.around(np.array(target), 2))
                    targets_org = targets_org.reshape(-1).float().cpu().numpy().tolist()
                    targets_org = list(np.around(np.array(targets_org), 2))

                 #   target2 = target.reshape(-1).float().cpu().numpy().tolist()
                 #   target2 = list(np.around(np.array(target2), 2))
                 #  targets_org2 = targets_org.reshape(-1).float().cpu().numpy().tolist()
                 #  targets_org2 = list(np.around(np.array(targets_org2), 2))
                    
                    #print(targets_org2[2]-target2[2])
                #    for res in ["target {} : target_org {}".format(x,y) for x,y in zip(target,targets_org)] :
                #        print(res)
                           # print(decimal.Decimal.from_float(float(target[i])))
                           # print(decimal.Decimal.from_float(float(targets_org[i])))                    
                   ...
                   # print("////")
                    ...
                    targets_org = torch.FloatTensor(targets_org)
                    #print(torch.equal(targets_org2, target2))
                    if cuda:
                        output = output.cuda()
                        target = target.cuda()
           ...
           #print(y_pred_tag)
            ...
            preds += y_pred_tag.reshape(-1).float().cpu().numpy().tolist()
            targets += target.reshape(-1).float().cpu().numpy().tolist()
            
        #redirada Confusion Matrix 
        #ver como apresentar os dados
...
#    print("////")
    print("MSE:", mean_loss) # removido Acurracy
...
def run_test(args, checkpoint_path, testloader, c, model_name, ap, cuda=True):
    ...
    # define loss function //provavelmente mudar aqui pip install pytorch-ignite
...
if __name__ == '__main__':
    ...
    run_test(args, args.checkpoint_path, test_dataloader, c, c.model_name, ap, cuda=False)
